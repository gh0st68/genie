#Genie 5.0 - by gh0st - visit irc.twistednet.org Channel #dev
...
if os.path.exists("config.json"):
    ...
    config["NICKNAME"] = input("Enter your nickname: ")
    config["CHANNEL"] = input("Enter the channel to join (e.g., #Twisted): ")
    config["OWNERS"] = input("Enter owner nicks (comma-separated, e.g., gh0st,n0ne`,mits`,sukitfup): ").split(',')
    config["TEMPERATURE"] = float(
        input("Enter the temperature value (e.g., 0.5 for more deterministic, 1.0 for default, 1.5 for more random): "))
    ...
    def on_pubmsg(self, connection, event):
        try:
            ...
            if message.lower().startswith(f"{nickname_lower} ") or \
                ...
                response = self.get_chatbot_response(message_without_bot)
